...
export class CloudposReportComponent implements OnInit {
  ...
  roleMenu: any[] = [];
  selectedRoleName: any | null = null;
  allUsers: any[] = [];
  allPrivilege: any[] = [];
  ...
  constructor(
    private readonly cloudPosReportService: CloudposReportService,
...
private initCompanyInfoCreateForm(): void {
 ...
 getCompanyInfo(): void {
  ...
  getAllUser(): void {
    this.cloudPosReportService.getAllUser().subscribe({
      next: (res) => {
        this.allUsers = res ?? [];
      },
      error: (error) => {
  ...
  addRole(): void {
    if (this.selectedRole) {
      ...
      const dto = {
        ID: 0,
        ROLE_NAME: this.addedRole,
        MENULISTID: null,
      };
      this.cloudPosReportService.addRole(dto).subscribe({
        next: (res) => {
  ...
  menuOnSubmit(event?: Event, myForm?: NgForm) {
    ...
    this.cloudPosReportService.menuOnSubmit(dto).subscribe({
      next: (res) => {
        const isSuccess = res?.success === true;
  ...
  assignMenu(Role_Name: any, menuModal: any): void {
    this.spin = true;
    debugger;
    this.cloudPosReportService.assignMenu(Role_Name).subscribe({
      next: (data: any) => {
        this.roleMenu = data;
        console.log('Projects loaded:', this.roleMenu);
        this.spin = false;
        this.selectedRoleName = Role_Name;
  ...
  loadParentMenus(): void {
    this.cloudPosReportService.GetParentNavCloudPosDBKMART().subscribe({
      next: (res) => {
  ...
  onSubmit(): void {
    ...
    const request = isEdit
      ? this.cloudPosReportService.updateNav(formData)
      : this.cloudPosReportService.createNav(formData);
  ...
  getRole() {
    this.cloudPosReportService.getRoleCloudPosDBKMART().subscribe({
      next: (data: any) => {
  ...
  onPrivilegeSubmit() {
    ...
    this.cloudPosReportService.assignedUserMenus(dto).subscribe({
      next: (res) => {
        const isSuccess = res?.success === true;
