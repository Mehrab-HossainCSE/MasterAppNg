...
onSubmit(): void {
  ...
  const isEdit = !!(this.userForm as any).editingId;
    ...
    next: (res: any) => {
      ...
      if (isSuccess) {
        if (isEdit) {
          ...
          this.swalOptions.text = res?.Messages?.[0] ?? 'User updated successfully.';
          this.swalOptions.icon = 'success';
          this.getAllMasterUser();
        } else {
          this.swalOptions.title = 'Created!';
          this.swalOptions.text = res?.Messages?.[0] ?? 'User created successfully.';
          this.swalOptions.icon = 'success';
          this.getAllMasterUser();
        }
      } else {
...
getProjects(userId: string) {
  this.cloudPosService.getProjects(userId).subscribe({
    next: (res: any) => {
      ...
      console.log("Initial ProjectListId:", this.projectForm.value.ProjectListId);
